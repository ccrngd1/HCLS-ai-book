---
id: ch10-r05-findings
title: 'Resolve open findings: Recipe 10.5: Patient-Facing Voice Assistant ⭐⭐'
...
- type: file_exists
  ...
  paths:
  - chapter10.05-architecture.md
  - chapter10.05-patient-facing-voice-assistant.md
  - chapter10.05-python-example.md
- type: shell
  name: findings-guardrail
  commands:
  - python3 check_findings.py chapter10.05
- type: persona_review
  name: findings-resolved
  persona: TechExpertReviewer
  pass_condition: >-
    Every open finding listed in chapter10.05-todo.md has been either (a) resolved
    ...
    security regressions. Resolved items removed from chapter10.05-todo.md. No new em dashes are introduced into the recipe source files.
---


## Objective
Resolve the open expert-review and code-review findings for Recipe 10.5: Patient-Facing Voice Assistant ⭐⭐, listed in `chapter10.05-todo.md`.

## Inputs
- `chapter10.05-todo.md` (the open findings checklist).
- Reviewer context: `reviews/chapter10.05-expert-review.md`, `reviews/chapter10.05-code-review.md`.
- Recipe source files:
  - `chapter10.05-architecture.md`
  - `chapter10.05-patient-facing-voice-assistant.md`
  - `chapter10.05-python-example.md`
...
## Instructions
1. Read `chapter10.05-todo.md` (and any review files above) for full context on each finding.
...
4. Remove each resolved entry from `chapter10.05-todo.md`.
5. If a finding needs an external citation you cannot verify, or a product decision only the author can make, leave it in `chapter10.05-todo.md` prefixed with `[NEEDS HUMAN]` and a one-line reason. Do not guess.
