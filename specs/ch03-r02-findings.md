---
id: ch03-r02-findings
title: 'Resolve open findings: Recipe 3.2: Patient No-Show Pattern Detection ⭐'
...
- type: shell
  ...
  commands:
  - python3 check_findings.py chapter03.02
- type: persona_review
  name: findings-resolved
  persona: TechExpertReviewer
  pass_condition: >-
    Every open finding listed in chapter03.02-todo.md has been either (a) resolved
    ...
    security regressions. Resolved items removed from chapter03.02-todo.md. No new em dashes are introduced into the recipe source files.
---


## Objective
Resolve the open expert-review and code-review findings for Recipe 3.2: Patient No-Show Pattern Detection ⭐, listed in `chapter03.02-todo.md`.

## Inputs
- `chapter03.02-todo.md` (the open findings checklist).
- Reviewer context: `reviews/chapter03.02-expert-review.md`, `reviews/chapter03.02-code-review.md`.
- Recipe source files:
  - `chapter03.02-architecture.md`
  - `chapter03.02-patient-no-show-pattern-detection.md`
  - `chapter03.02-python-example.md`
...
## Instructions
1. Read `chapter03.02-todo.md` (and any review files above) for full context on each finding.
...
4. Remove each resolved entry from `chapter03.02-todo.md`.
5. If a finding needs an external citation you cannot verify, or a product decision only the author can make, leave it in `chapter03.02-todo.md` prefixed with `[NEEDS HUMAN]` and a one-line reason. Do not guess.
