---
id: ch14-r05-findings
title: 'Resolve open findings: Recipe 14.5: Operating Room Block Scheduling'
...
- type: file_exists
  ...
  paths:
  - chapter14.05-architecture.md
  - chapter14.05-operating-room-block-scheduling.md
  - chapter14.05-python-example.md
- type: shell
  name: findings-guardrail
  commands:
  - python3 check_findings.py chapter14.05
- type: persona_review
  name: findings-resolved
  persona: TechExpertReviewer
  pass_condition: >-
    Every open finding listed in chapter14.05-todo.md has been either (a) resolved
    ...
    security regressions. Resolved items removed from chapter14.05-todo.md. No new em dashes are introduced into the recipe source files.
---


## Objective
Resolve the open expert-review and code-review findings for Recipe 14.5: Operating Room Block Scheduling, listed in `chapter14.05-todo.md`.

## Inputs
- `chapter14.05-todo.md` (the open findings checklist).
- Reviewer context: `reviews/chapter14.05-expert-review.md`, `reviews/chapter14.05-code-review.md`.
- Recipe source files:
  - `chapter14.05-architecture.md`
  - `chapter14.05-operating-room-block-scheduling.md`
  - `chapter14.05-python-example.md`
...
## Instructions
1. Read `chapter14.05-todo.md` (and any review files above) for full context on each finding.
...
4. Remove each resolved entry from `chapter14.05-todo.md`.
5. If a finding needs an external citation you cannot verify, or a product decision only the author can make, leave it in `chapter14.05-todo.md` prefixed with `[NEEDS HUMAN]` and a one-line reason. Do not guess.
