...
- type: shell
  ...
  commands:
  - 'python3 -c "import re,sys,pathlib; t=pathlib.Path(''chapter03.04-medication-dispensing-anomalies.md'').read_text(encoding=''utf-8'');
    ids=[''A1'', ''A2'', ''A3'', ''A4'', ''S1'', ''S2'', ''S5'']; missing=[i for i
...
- type: shell
  name: auto-fix-style
  commands:
  - python3 fix_style.py chapter03.04-medication-dispensing-anomalies.md
- type: persona_review
  name: findings-resolved
  persona: TechExpertReviewer
  pass_condition: All findings listed in the Instructions section (A1, A2, A3, A4,
    S1, S2, S5) are either resolved in the recipe text or explicitly closed with a
