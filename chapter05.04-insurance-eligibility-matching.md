...
A second difference: the match often has to be real-time. When a patient registers at the front desk, the system has 2-5 seconds to come back with an eligibility answer that the staff can act on.  That eliminates a lot of the techniques you can use in batch. No expensive ML inference. No multi-step reasoning loops. No human review in the critical path. The real-time path has to be fast and decisive, with the harder cases routed to an asynchronous review queue rather than blocking the registration flow.
...
The HIPAA-mandated electronic standard for eligibility verification is the X12 270 (eligibility inquiry) and X12 271 (eligibility response) transaction set.  Almost every electronic eligibility verification in US healthcare flows through this standard, either directly between payer and provider, or (more commonly) through a clearinghouse intermediary.

The 270 inquiry contains the patient demographics, the payer identifier, the requesting provider's National Provider Identifier (NPI), the date of service, and (optionally) one or more service-type codes specifying what coverage is being asked about. The 271 response contains the matched member's coverage details: active/inactive flag, coverage start and end dates, plan name and identifier, copay and coinsurance information for the requested service types, deductible status, and any informational segments the payer chooses to include.
...
**Search match.** The 270 omits the member ID and provides patient demographics (name, DOB, sometimes SSN, sometimes address). The payer attempts a probabilistic search against their member rolls and returns the best match (or none). This is the fallback when the patient does not have a card, when the card has the wrong member ID, when the member ID is unknown to the front desk, or when the practice management system is doing a sweep across the population to refresh eligibility data without making the patient produce their card. 
...
**Eligibility APIs based on Fast Healthcare Interoperability Resources (FHIR) are emerging alongside X12.** The CMS Patient Access API, the CMS Provider Directory API, and the broader Da Vinci Project have produced FHIR-based alternatives to the X12 270/271 flow for some use cases.  The X12 standard is not going away (it is the regulatory baseline and the broad payer infrastructure is built around it), but FHIR-native eligibility verification is starting to appear, particularly for new use cases like payer-to-payer data exchange and patient-facing apps.
...
- **Recipe 5.5 (Cross-Facility Patient Matching for a health information exchange (HIE)):** Eligibility data from one facility's payer is useful context for cross-facility matching at HIE boundaries; the eligibility-match store supplies that context.
...
- **Recipe 1.1 (Insurance Card Scanning):** The patient-portal coverage self-service flow uses optical character recognition (OCR) on card images to extract member IDs that feed this matcher.
- **Recipe 1.8 (Explanation of Benefits Processing):** Explanation of benefits (EOB) documents are the downstream artifact of claims that depended on eligibility matches; tying EOB processing back to the originating eligibility match closes the revenue-cycle loop.
