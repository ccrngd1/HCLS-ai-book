...
**FHIR-based eligibility APIs are emerging alongside X12.** The CMS Patient Access API, the CMS Provider Directory API, and the broader Da Vinci Project have produced FHIR-based alternatives to the X12 270/271 flow for some use cases.  The X12 standard is not going away (it is the regulatory baseline and the broad payer infrastructure is built around it), but FHIR-native eligibility verification is starting to appear, particularly for new use cases like payer-to-payer data exchange and patient-facing apps.
