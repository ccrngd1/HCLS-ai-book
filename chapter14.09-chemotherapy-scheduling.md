...
FUNCTION optimize_schedule(requests, resource_model, config):
    ...
    // Constraint: drug stability (batch planning guard)
    // The real drug stability risk is reactive: a patient delayed after prep
    // causes the bag to expire. Step 6 handles that in real time. But the
    // batch planner still validates that the planned prep-to-administration
    // gap (including a buffer for expected variance) fits within the BUD.
    FOR EACH request IN requests:
        prep_start = request.start_var - request.pharmacy_prep_minutes
        prep_completion = prep_start + request.pharmacy_prep_minutes
        model.add_constraint(
            request.start_var - prep_completion
            + config.expected_delay_buffer_minutes
