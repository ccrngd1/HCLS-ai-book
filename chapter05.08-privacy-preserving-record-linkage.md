...
<!-- TODO (TechWriter): Specify the salt-management ceremony at the architectural level: the multi-party HSM ceremony that generates the salt, the dual-control approval pattern (two HSM operators from non-overlapping organizations must approve the rotation), the audit-log schema for salt-related operations, the catch-up-window policy with an explicit duration and access-control constraint, the post-rotation re-encoding-coverage SLA per participant, and the monitoring of re-encoding completion against the salt-rotation deadline. -->
