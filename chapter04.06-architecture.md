...
- **Measures with ambiguous numerator definitions.** Some measure specifications have been written in ways that admit multiple interpretations. Different vendors' implementations of the "same" Healthcare Effectiveness Data and Information Set (HEDIS) measure can produce different numerators by 1 to 3 percentage points. The recipe's measure registry encodes one interpretation; reconciling with the plan's PBM/HEDIS-vendor numerators requires explicit alignment work and ongoing comparison.
