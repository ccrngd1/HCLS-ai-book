...
**End-to-end speech models are getting wider deployment in telephony.** Older IVR stacks bolted ASR onto a separate NLU service. Newer stacks increasingly fuse them into single models that go from audio to intent directly. The accuracy benefit is real; the operational pattern is converging. You'll see "spoken language understanding" (SLU) as a label for this. 
