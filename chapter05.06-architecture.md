...
**Amazon HealthLake for the Fast Healthcare Interoperability Resources (FHIR)-native clinical view.** Where the institution has an OMOP CDM substrate, the linkage outputs feed OMOP. Where the institution has a FHIR-native data lake, HealthLake stores the clinical encounters, observations, and procedures as FHIR resources, and the linkage output joins claims to the FHIR Encounter resource.  The linkage architecture treats the FHIR-native and OMOP-native representations as alternative downstream targets; the link itself is upstream.
...
> - The [Health Level Seven (HL7) FHIR ExplanationOfBenefit resource](https://www.hl7.org/fhir/explanationofbenefit.html) is the FHIR-native representation of post-adjudication claims data and the substrate for the CMS Patient Access API claims feeds.
