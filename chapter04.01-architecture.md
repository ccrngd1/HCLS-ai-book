...
**Portal push notifications: a HIPAA-constrained channel.** If your organization has a patient-facing mobile app with push notifications (via Amazon SNS Mobile Push to APNs/FCM), you can include it as a candidate channel. However, Apple Push Notification service (APNs) and Google Firebase Cloud Messaging (FCM) are not typically covered under a business associate agreement (BAA). This means you cannot send protected health information (PHI) in the push payload itself. The architecture pattern that works: send a content-free notification ("You have a new appointment reminder; tap to view") that triggers the app to fetch the full reminder details from your BAA-covered backend via an authenticated API call. The app renders the details locally after the patient unlocks and authenticates. Delivery events flow back via SNS Mobile Push platform attributes and CloudWatch metrics on the platform application endpoint. This adds latency (the patient must open the app to see details) but keeps PHI within your BAA boundary.
