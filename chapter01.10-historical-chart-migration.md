...
The regulatory pressure keeps mounting. CMS Interoperability and Patient Access rules require payers to make member data available via Fast Healthcare Interoperability Resources (FHIR) APIs, and "member data" includes longitudinal history. Healthcare Effectiveness Data and Information Set (HEDIS) and Stars programs reward complete longitudinal records. The HIPAA Right of Access means members can request their complete records, including the ones still sitting in archive boxes. The argument for deferring this work is getting harder to make every year.
