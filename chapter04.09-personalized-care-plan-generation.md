...
These three primitives are the structured representation the rest of the system manipulates, and the direction matters: the narrative the patient and clinicians read is *rendered from* the structure, not the other way around. Structured-then-narrative is what makes the plan auditable, queryable, updatable, and amenable to fairness analysis, and it is what keeps the LLM from becoming the system of record for clinical decisions. This is well-trodden ground: Fast Healthcare Interoperability Resources (FHIR)'s `Goal`, `CarePlan`, `ServiceRequest`, and `Task` resources implement essentially this graph, and C-CDA and CMS care-plan templates map to the same structure. What differs across implementations is the richness of the graph and how dynamically it is maintained.
...
**Regulatory posture is set early.** In the United States the relevant machinery is the 21st Century Cures Act carve-out for non-device clinical decision support, which FDA interprets through its Clinical Decision Support Software guidance (revised and reissued in January 2026, superseding the 2022 version). The carve-out has four criteria and software must satisfy all of them to sit outside the device definition. The one that usually decides a care-plan generator is the requirement that the recommendation be directed to a *health care professional* who can independently review the basis for it, rather than relying primarily on it. That is why the review posture is a regulatory decision and not just a workflow preference: where the care team reviews and modifies the plan before activation, the support is clinician-mediated and the framing resembles other care-management workflows, and where the plan reaches the patient with minimal review, the HCP-directed criterion is no longer met and the analysis tightens toward FDA SaMD regulation depending on the clinical claims in the patient-facing narrative.

The 2026 revision is generally more permissive than what it replaced, notably in treating a single specific recommendation as less automatically device-triggering than the earlier guidance did, which matters for a system whose whole output is one specific plan. It also puts more emphasis on automation bias. None of that changes the shape of the decision: most deployments should err toward care-team review, and teams pursuing more direct-to-patient delivery should buy regulatory analysis early rather than discovering the boundary after launch. Confirm current guidance with counsel; this area has moved twice in recent years and will move again.
...
**Buy versus build: buy the care-management platform, build the generation layer inside it.** There is no mature market for AI-generated personalized care plans, so if you want this, you will be building the generation. What you should not build is the surrounding platform. Care-management modules in the major EHRs and the standalone population-health vendors already handle plan storage, task routing, outreach, and documentation, and rebuilding that is a multi-year detour from the part you actually care about. Two constraints shape the build. First, as the regulatory discussion above sets out, keeping the care team between the generator and the patient is what keeps you inside the non-device carve-out, so design for clinician mediation from the start rather than retrofitting it later. Second, the structured-then-narrative separation described below is what makes a generated plan auditable, and auditability is what makes it defensible. Buy the plumbing, build the reasoning, and keep a clinician in the loop.

Personalized care plan generation is the recipe in this chapter where the gap between "the system produces a plan" and "the system produces a plan that meaningfully changes care" is the widest. Every health system, every payer, and every care management vendor has, at some point, shipped a care plan generator. Most of those generators produce a document. The document goes into the electronic health record (EHR). The document is opened occasionally. The document is updated rarely. The document does not, in any honest accounting, change what happens to the patient. The architecture in this recipe is largely about the operational, content, and engagement discipline that distinguishes a plan that changes care from a plan that occupies disk space. Most of that discipline is not AWS-specific. Most of it is clinical content choices made well, governance applied seriously, multi-actor orchestration designed deliberately, and LLM constraints enforced strictly. The cloud infrastructure is comparatively easy.
...
- **Recipe 2.x (Clinical Text Generation):** The narrative generation and validator pattern uses techniques developed across Chapter 2; the validator pattern from 2.5 (After-Visit Summary) and 2.9 (Clinical Decision Support Synthesis) applies directly.
- **Recipe 7.x (Predictive Risk Modeling):** Risk-stratification scores feed the plan input record; high-risk patients get more frequent plan review cadences and more action-focused plans.
- **Recipe 12.x (Forecasting & Time-Series Analysis):** Disease-trajectory forecasting from Chapter 12 informs the goal horizons and the action timing.
- **Recipe 13.x (Knowledge Graphs & Clinical Reasoning):** The clinical content library, with relationships between conditions, goals, actions, contraindications, and guideline references, is naturally modeled as a knowledge graph at higher sophistication levels.
- **Recipe 11.x (Conversational AI & Virtual Agents):** Patient-facing conversational interfaces can deliver and adapt the plan interactively, leveraging the structured plan as the source of truth for the conversation.
