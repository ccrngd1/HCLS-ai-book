...
**Amazon EventBridge for outcome capture and feedback events.** When the appointment outcome (completed, no-show, cancelled, rescheduled) is recorded in the electronic health record (EHR), an event flows to EventBridge. A Lambda consumer joins the outcome to the original prediction record and writes the label to the feature store and the label archive. EventBridge Scheduler triggers the retraining job on the configured cadence.
...
**Subgroup monitoring is not optional.** The model will perform differently across subgroups. That is a matter of fact, not hypothesis. The dashboard that surfaces subgroup area under the curve (AUC), subgroup intervention rates, and subgroup outcome improvement is part of the minimum viable deployment, not a post-launch enhancement. If your organization captures race/ethnicity and preferred language, include them in the dashboard. If not, include age bands, insurance type, and clinic location as proxy subgroups; these don't fully cover the concern but they're better than nothing.

**Provider-level patterns deserve their own dashboard view.** If the model consistently flags one provider's panel as high risk, the problem may not be the patients. It may be scheduling practices: always booking new patients into early-morning slots, over-reliance on long lead times, or inflexible visit durations that push patients toward no-showing rather than calling to reschedule. A provider-level summary (aggregate no-show rate, average risk score, intervention rate, and outcome improvement) is a useful companion to the patient-level model. It helps operations teams distinguish "these patients are hard to reach" from "this provider's scheduling template doesn't match their patient population." Treat this as a standard QuickSight view grouped by `provider_id`, refreshed alongside the subgroup dashboard.
