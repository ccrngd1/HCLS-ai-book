...
**Intervention chaining: the state-machine version.** The intervention catalog carries `predecessor_intervention_id` and `successor_intervention_id` fields on each intervention record that participates in a chain. When the allocator assigns the first link, the recommendation log records `chain_id` (a UUID shared across the chain) and `chain_position` (1, 2, 3...). The engagement-attribution Lambda in Step 8 watches for `intervention_completed` events. When it sees one, it checks whether the completed intervention's catalog record has a `successor_intervention_id`. If so, it enqueues the successor intervention for the same (patient, therapeutic_class) with the same `chain_id` and an incremented `chain_position`. The successor goes through the eligibility filter and the consent check (the patient's state may have changed since the chain started), but bypasses the priority scoring and allocation steps since it was pre-approved as part of the chain when the first link was allocated. If the predecessor intervention fails or the patient declines, the chain terminates and the status is recorded on the chain record.

The simpler sequence version skips the state machine: allocate both links in the same run, mark the second with `hold_until = predecessor.tracking_id + completion`, and let the orchestrator check the hold condition before dispatching. This works for two-link chains but does not generalize to longer sequences or to chains where the successor depends on the predecessor's outcome (e.g., "if cost-assistance succeeded, enroll in reminder; if cost-assistance failed, escalate to care manager").
...
- **PCP and care-team summaries.** Same pattern as 4.4's PCP briefings. A structured summary of the patient's adherence picture, the recommended intervention, and the rationale, written into the electronic health record (EHR) inbox or care-team dashboard.
...
- **Recipe 8.x (Clinical natural language processing (NLP) & Information Extraction):** Pharmacist consult notes can be parsed by NLP techniques to extract structured barrier labels; the labels feed the supervised barrier classifier. Free-text-to-structured-label pipelines are a Chapter 8 problem.
