...
**Model versioning and rollback.** When you retrain, the new model may be worse than the old one. You need automated validation before promoting a new model to production: score the calibration set and check that area under the curve (AUC) and ECE meet thresholds. If validation fails, keep the previous model running and alert the ML team.
