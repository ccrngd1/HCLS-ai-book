...
Note that patient-level protected health information (PHI) is loaded into SageMaker Processing ephemeral storage during case-mix model training (the regression models require patient-level features like HCC scores and demographics). This data exists only for the duration of the Processing job and is automatically deleted when the job completes, but it means your SageMaker Processing configuration must enforce volume encryption (`VolumeKmsKeyId`) and VPC mode with no internet access. Apply the minimum panel size filter (30-50 patients depending on specialty) before model training, not just before clustering. This prevents small-panel provider features from encoding individual patient characteristics into the adjustment model's training set.
