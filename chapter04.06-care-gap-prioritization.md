...
- **Quality-measure-defined gaps.** The patient is in the denominator of a HEDIS, Stars, ACO, or contracted quality measure, and the numerator condition (the qualifying event or procedure within the lookback window) is not satisfied. These are deterministically computable from claims, lab data, and EHR data given the measure specification. Examples: HEDIS Eye Exam for Patients with Diabetes (EED), BCS-E breast cancer screening, FUH (Follow-Up after Hospitalization for Mental Illness), CCS cervical cancer screening. (Note: NCQA retired the parent Comprehensive Diabetes Care (CDC) measure beginning HEDIS MY 2022 and split it into separate measures: EED, KED (Kidney Health Evaluation), GSD (Glycemic Status Assessment), and BPD (Blood Pressure Control). If you're working from pre-2022 references, map them to the new measure IDs.) The measure specifications are publicly published, are updated annually, and have well-defined denominator and numerator definitions. 
...
Denominator logic involves age, sex, condition flags (diabetes diagnosis on the active problem list or recent International Classification of Diseases (ICD)-10 codes), enrollment criteria (continuously enrolled for X months), and exclusion criteria (palliative care, hospice, dialysis-dependent for some measures, pregnancy for others). Many denominators have specific look-back periods. Many use procedure or diagnosis code value sets that NCQA, CMS, or specialty societies publish and revise annually.
...
### Data Quality as a First-Class Signal

The gap evaluation step produces a `data_quality_flag` per (patient, measure) that captures source-completeness assessment: values include `complete`, `sparse_history`, `multi_source_disagreement`, `recent_plan_change`, and `cross_provider_fragmentation`. This flag is not metadata. It is an input to every downstream stage:

- The urgency scorer dampens confidence on non-`complete` cases (a confident urgency score on a patient whose data is fragmentary is overconfidence).
- The visit-context ranker suppresses low-quality gaps from the in-visit agenda (surfacing a "gap" that might already be closed at another provider wastes limited visit time).
- The async orchestrator routes low-quality cases to a verification-first pathway before any closure-specific outreach (call to confirm status before launching a reminder cascade).
- The closure tracker tightens (or, for `cross_provider_fragmentation`, relaxes) the canonical-source rule.
- The chase-team brief opens with verification framing when data quality is in doubt ("confirm whether patient has already had this screening elsewhere before proceeding with the gap-closure script").

The "calling a patient about a colonoscopy they had last week" failure mode that the Honest Take warns about is exactly what `cross_provider_fragmentation` flags. Not gating downstream decisions on the data quality flag produces precisely that failure. A confidently "open" gap on a patient with `cross_provider_fragmentation` is a confidently wrong recommendation if the patient already had the procedure at a non-network provider.
...
- **Recipe 7.x (Predictive Risk Modeling):** The clinical urgency model in this recipe is a risk-scoring problem; Chapter 7's risk-scoring patterns and validation methodology apply directly.
- **Recipe 8.x (Clinical natural language processing (NLP) & Information Extraction):** Clinician override notes can be parsed by NLP to extract structured override reasons more reliably than relying on dropdown selections; the structured labels feed urgency-model retraining.
- **Recipe 12.x (Forecasting & Time-Series Analysis):** Closure-rate forecasting and capacity planning across measures is a forecasting problem; Chapter 12 covers the techniques.
- **Recipe 13.x (Knowledge Graphs & Clinical Reasoning):** The measure registry is a structured knowledge artifact; advanced implementations can model measure relationships (one measure subsumes another, one closure satisfies multiple measures, certain combinations of closures imply additional gaps) as a knowledge graph rather than as flat measure records.
- **Recipe 14.x (Optimization & Resource Allocation):** The visit-context ranker and the async-allocator are heuristic versions of constrained-optimization problems; integer programming or column-generation can produce provably-optimal allocations when constraints multiply.
