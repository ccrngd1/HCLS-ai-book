...
**Upcoding and undercoding bias.** Some organizations systematically upcode (assign more specific or higher-severity codes than documented). Others undercode (use unspecified codes when specificity is available). Your model learns whatever bias exists in your historical data, and this is the one item on this list that is not merely a data quality problem. A model trained on upcoded history does not just inherit the tendency, it automates it and applies it uniformly to every encounter, which is how a tendency becomes a pattern. Codes unsupported by the documentation are the substance of most coding enforcement, and "the model suggested it" is not a defence, because the organisation submitting the claim is the one attesting to it. It is also self-documenting in a way human upcoding is not: your inference logs record exactly what the system recommended and how often a coder took the suggestion. So treat a drift toward higher-severity codes as a signal that belongs in front of your compliance officer, rather than as a metric the data science team quietly tunes. What your obligations are in that situation is a question for them and for counsel, not for this book.
