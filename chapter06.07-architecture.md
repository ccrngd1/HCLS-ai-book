...
**Amazon SageMaker for natural language processing (NLP) models.** The clinical NLP pipeline (entity extraction, negation detection, temporal reasoning) requires custom models trained on clinical text. SageMaker gives you the training infrastructure and real-time inference endpoints for those custom models. For organizations without custom models, Amazon Comprehend Medical provides pre-trained clinical NLP as a starting point, though it lacks the trial-specific fine-tuning that improves precision.
...
**AWS Glue and Amazon Athena for structured pre-screening.** The structured pre-screen is fundamentally a large-scale query against patient data. Glue extract, transform, and load (ETL) jobs transform electronic health record (EHR) extracts into a queryable format. Athena runs the eligibility queries against the transformed data without requiring a persistent database cluster. For a 180,000-patient registry with 40 structured criteria, Athena can complete the pre-screen in minutes.
