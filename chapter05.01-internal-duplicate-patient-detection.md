...
**A regulatory note, and its limits.** The no-link flags described above include records segmented under 42 CFR Part 2, and that rule changed underneath this design. The 2024 final rule, whose compliance date was 16 February 2026, moved Part 2 substantially closer to HIPAA: it permits a single patient consent covering future uses and disclosures for treatment, payment and operations rather than a separate consent each time, and it aligns Part 2 breach notification and penalties with HIPAA. It also preserves the long-standing bar on using these records in criminal or civil proceedings without consent or a court order. Two things follow for a matcher, and both are design questions rather than compliance ones: a single broad consent changes what a legitimate link looks like, so a permanent no-link flag may now be more conservative than the rule requires; and because breach notification is aligned with HIPAA, a wrong merge involving Part 2 records sits under the framing described earlier in this recipe. Which consents you actually hold, and what they permit, is a question for your privacy officer and counsel.
