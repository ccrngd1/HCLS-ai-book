...
**Effort:** 1 of 5 · **Maturity:** Proven · **Oversight:** Exception review
...
This is what duplicate patient records do. They are not, as the IT department sometimes tries to frame it, a data quality nuisance. They are a patient safety hazard with documented clinical consequences. The Joint Commission has flagged patient identification as a top patient safety concern essentially every year for the last two decades: "improve the accuracy of patient identification" was Goal 1 of the very first National Patient Safety Goals, effective 2003, and it survives today as NPSG.01.01.01, "identify patients correctly." ECRI has published a dedicated evidence review of patient-identification errors and their clinical consequences. The cost shows up in adverse drug events, repeated lab tests because the result from last visit isn't visible, missed care gaps because the screening was done on the other chart, denied claims because the insurance is on yet another record, and a steady drip of staff time spent reconciling things at the desk while patients wait.

The other thing duplicate records do is silently inflate every metric the organization reports. The patient count is wrong. The unique-patient denominators in Healthcare Effectiveness Data and Information Set (HEDIS) measures are wrong. The cohort sizes in any analytics work are wrong. The marketing list has duplicates. The patient portal sends three reminders to the same person who just clicks delete and wonders why their primary care provider is so disorganized. The financial reconciliation routinely finds the same human being with three open balances on three accounts. None of this is rare. AHIMA has long put the typical hospital duplicate rate at roughly 5 to 10 percent of stored records, with about 10 percent as the commonly cited average and some organizations materially worse; AHIMA has also reported that at some facilities up to 20 percent of records are not accurately matched within a single system. Across organizations it degrades sharply: AHIMA puts cross-facility error rates as approaching 50 percent, and Pew has found matching as low as 80 percent within one care setting and as low as 50 percent between organizations exchanging records. Larger organizations and health information exchanges sit at the worse end of every one of those ranges. 

The good news is that duplicate patient detection within a single system is the most tractable entity-resolution problem in healthcare. You have one source. You control the field formats. You can tune aggressively because the merge action is gated by human review. The techniques are well-understood. The tools have been around for decades. Most organizations could be doing this and aren't, or are doing it once-a-year as a manual cleanup project that gets behind the moment it ends.

This recipe builds the always-on version. Every new registration gets compared against the existing database. Suspected duplicates get queued for review. Known matches get merged with full audit and reversibility. The blocking, similarity, and probabilistic scoring infrastructure you build here easily lets you build many other use cases in this category.
...
This is the unglamorous half of duplicate detection. When you merge two records, the merged record needs concrete field values. Which name? Which address? Which phone number? Which insurance? Which problem list?
...
**A regulatory note, and its limits.** Some of the no-link flags above cover records held under 42 CFR Part 2. That is the rule protecting substance use disorder treatment records, which also cannot be used in court without consent or a court order. The rule changed in 2024, and the compliance date was 16 February 2026.

Two of those changes matter for a matcher. Part 2 used to require a separate patient consent for each disclosure; it now allows one consent to cover future disclosures for treatment, payment and operations. And Part 2 breach notification now follows the same rules as HIPAA.

Each has a design consequence. Where a patient has given the broader consent, a permanent no-link flag may be stricter than the rule now requires, so it is worth asking whether yours is set by law or by habit. And because the breach rules are now aligned, a wrong merge involving these records falls under the breach framing described earlier in this recipe.

None of that tells you what you owe. Which consents you actually hold, and what they permit, is a question for your privacy officer and counsel.

### Where the Field Has Moved

A few practical updates worth knowing:

- **Open-source tooling is mature, but not uniformly maintained.** Splink is the safe default: actively developed by the UK Ministry of Justice's analytical services team, past eight million downloads, interpretable Fellegi-Sunter output with EM-based parameter estimation, and backends spanning DuckDB for local work through Spark, Athena, and Postgres for scale. Zingg and dedupe are credible alternatives, and newer entrants keep appearing. The Python Record Linkage Toolkit (`recordlinkage`) is still useful and widely taught but has seen much less release activity than Splink. Check each project's recent commit and release history before you build on it; in this corner of the ecosystem the gap between "well known" and "currently maintained" is wider than usual. 
...
**Blocking is the recall-vs-cost knob.** Multiple blocking passes increase recall at the cost of more candidate pairs to score. Tighter blocking keys reduce candidate count at the cost of missing some real duplicates. The right answer is empirical: pick an initial set of passes, measure recall against a labeled gold set, add or tighten passes until recall is acceptable, and accept the resulting candidate count. 
...
**Cohort-stratified accuracy monitoring is part of the system, not an afterthought.** Compute and report match rate, false-positive rate, and review queue depth by demographic cohort (race, ethnicity, language, age band, geographic region, primary-language). Significant disparities (worse match rate for Hispanic patients than non-Hispanic, for example) are signals that the comparators or the m/u probabilities are not generalizing across populations and need cohort-specific tuning. Define explicit disparity alert thresholds: per-cohort recall ratio (worst vs. best cohort), auto-match precision ratio, post-merge unmerge rate ratio, and review-queue depth-per-FTE ratio. Example: `MATCH_RATE_DISPARITY_THRESHOLD = 0.10` fires an alert when the worst-performing cohort's recall falls below 90% of the best-performing cohort's recall. Per-axis threshold overrides are governed by an equity-review committee. When a cohort's sample size drops below a minimum (typically 50 pairs in the gold set), chronic suppression of that cohort's metrics is itself a fairness signal indicating the gold set needs augmentation for that population. The documented diagnose-and-address workflow fires when any alert crosses threshold: identify the contributing comparator or blocking pass, propose cohort-specific tuning, validate on the cohort-stratified gold set, and promote only after committee approval.

When emitting cohort-dimension metrics to the operational dashboard, use bucketed non-reversible cohort labels (e.g., cohort A, B, C, D, E, unknown) rather than raw demographic attributes. The cohort-label-to-attribute mapping lives in a separate access-controlled store loaded only at dashboard-render time. This prevents demographic inference from the metric stream itself.

**No-link flags protect safety-sensitive populations.** Certain patient records must never be auto-linked, regardless of how high they score. Populations requiring link protection include: patients in state Address Confidentiality Programs (Safe at Home), federal Witness Security participants, sealed adoption records, patients who have requested record separation (gender transition, protected name change), records segmented under 42 CFR Part 2 (substance use disorder, behavioral health), and explicitly-flagged family relationships (twins, parent-infant pairs sharing demographics). The pipeline consults a `no_link_flags` table at three stages: candidate generation (filter flagged records from standard blocking passes), threshold routing (bypass auto-match for any pair containing a flagged record), and review-queue assignment (route flagged-record pairs to a restricted queue staffed only by privacy-office and HIM-leadership roles). Flags are write-protected to privacy-office roles with separate-key encryption. This architectural primitive carries forward to recipes 5.5, 5.7, 5.9, and any cross-organizational matching scenario.

**Family-aware blocking prevents twin and parent-infant false merges.** Twins share birthday, last name, address, and sometimes first-name initial. The standard blocking passes will produce candidate pairs for twins in almost every pass; the standard comparators will score them high on the shared fields. Without a family-aware guard, these pairs regularly auto-merge. The mitigation: consult the `no_link_flags` table for `family_relationship_explicit` entries and skip comparison of explicitly-flagged family pairs. For pairs that enter scoring through other paths, down-weight comparator scores on shared-family fields (address, phone, insurance) when other fields suggest different individuals.
...
**Buy versus build: this is one of the few where building is right.** Commercial EMPI products exist, and they are a reasonable purchase if you want the problem owned by someone else. But this is the rare healthcare AI use case with a genuinely strong build argument, for two reasons. First, the hard part is not the algorithm, it is the calibration: the m and u probabilities, the blocking strategy, and above all the auto-match threshold are properties of *your* registration data and *your* risk tolerance. No vendor can set them for you, and you will be doing that tuning work whether you buy or build. Second, as noted below, the components compound. The blocking, comparators, probabilistic combiner, review queue, and survivorship rules you build here are the foundation for cross-facility matching, claims-to-clinical linkage, and longitudinal matching across name changes. A closed EMPI gets you the first use case and leaves you starting over on the next four. With Splink as a mature, actively maintained combiner, a competent team can own this. The honest caveat: "build" here means building on open source and committing to permanent ownership, not writing a matcher from first principles. If you cannot staff the permanent owning team described below, buy.

Internal duplicate patient detection is the recipe in this category with the highest payoff-per-engineering-hour ratio. The duplicate rate in the source database is silently costing the organization money and patient safety every day, and the project to fix it pays for itself in months in most cases.

The infrastructure here compounds and can be leveraged with more complex use cases. The blocking, comparators, probabilistic combiner, review queue, survivorship rules, and audit trail you build here can be directly re-used for things like cross-facility matching, claims-to-clinical, longitudinal across name changes, and more.  
...
The thing I would recommend to focus on is to invest more in the upstream registration-desk data-quality work in parallel with the matcher build. The matcher is a downstream system that compensates for upstream data quality issues. Every percentage-point improvement in DOB capture at the registration desk reduces the matcher's load by more than a percentage point, because the marginal records that lacked DOB are also typically the records that lacked other identifying fields, so the gain compounds. The two projects (registration data quality, matcher build) reinforce each other; running them sequentially leaves value on the table. Plan them together.
...
Several sibling recipes reuse the blocking, comparator, and review-queue infrastructure built here. Recipe 5.2 applies these primitives to the National Provider Identifier (NPI) registry; recipe 5.5 extends the framework to a health information exchange (HIE); the master patient index (MPI) that this recipe produces is the anchor for the personalization and predictive-modeling recipes in later chapters.

- **Recipe 5.2 (Provider NPI Matching):** Sibling Simple-tier recipe; uses similar string-similarity and probabilistic-linkage techniques against the NPI registry. The infrastructure (blocking, comparators, review queue) overlaps substantially.
