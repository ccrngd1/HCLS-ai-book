...
**Bayesian hierarchical models.** When the data has structure (cases nested within geographies, geographies nested within regions, weeks nested within seasons), hierarchical Bayesian models can borrow strength across the hierarchy and handle small-cell estimation gracefully. INLA-based approaches and MCMC-based approaches both have practitioners. MCMC-based R packages including `bsts` (Bayesian Structural Time Series, which uses its own C++ MCMC sampler) and `brms` (Stan-based, with appropriate priors) provide accessible implementations, as does the `INLA` R package for approximate Bayesian inference. 
