...
The orthopedic group that re-submitted a batch of claims after their practice management software choked on an X12 837 claim submission and they never got an X12 277 claim status response back. They thought the first batch got lost. It didn't. They sent it twice. Both batches paid before anyone noticed.

The hospital that bills primary care and inpatient on two different tax IDs. Same physician, same patient, same date of service, same Current Procedural Terminology (CPT) code for a critical care hour. Two payments. The physician doesn't even know; her billing is handled by a service.
...
**Buy versus build: you already own most of this, so build only the residual.** If you are a payer of any size you already license a claims editor, and your clearinghouse applies duplicate logic before claims even reach you. Exact and near-exact duplicates are a solved commercial problem, and rebuilding that layer is pure waste. The build case is narrow and specific: the fuzzy residual your editor misses, where the service line was re-coded, the date shifted by a day, or the rendering provider changed between submissions. Start by measuring what your existing editor already catches, because that number decides whether this project is worth staffing at all. Then build the blocking and scoring described here on top of your editor's output, not instead of it.

The blocking layer is where the leverage is, not the scorer. Teams new to this problem tend to obsess over the scoring model: which algorithm? XGBoost or LightGBM? Can we use embeddings? Most of the practical win comes from getting blocking right. A sloppy blocker that misses 20% of true duplicate pairs puts a ceiling on your recall that no scorer can recover. A careful blocker with multi-key lookup pushes that ceiling above 95%, at which point the scorer's job is easy. Budget accordingly.
...
What I recommend to any team looking to embark on this project, start with a deterministic exact-duplicate check running in production before anything else. It's a one-week project. It catches the obvious duplicates. It generates the first labels you'll need to train the fuzzy scorer. And it demonstrates value to the organization before you've invested in the harder pieces. Starting with the full ML pipeline before you've shipped the exact-match check is a classic overbuild, and I've seen multiple teams do it. 
