...
The mathematical goal: find the policy π that maximizes the expected sum of discounted future rewards. In notation: π* = argmax E[Σ γ^t * r_t], where γ is a discount factor that weights near-term rewards more heavily than distant ones.
...
- TODO: Verify current URL for MIMIC-IV dataset access (PhysioNet)
