...
**Stage 4: Feature joining.** For each candidate, attach the features the ranker needs: distance from the patient, network tier within the plan, claims-derived patient overlap (does this patient have prior visits with this provider?), provider quality scores (Healthcare Effectiveness Data and Information Set (HEDIS) performance, patient satisfaction if available), availability metrics (next-available appointment, panel openness), specialty-fit score from the query understanding stage, fairness controls.
