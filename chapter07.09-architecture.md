...
> **Watch out: Lambda timeouts on your sickest patients.** The irony of the timeout problem is that it hits hardest on long-stay ICU patients, who are exactly the ones who most need accurate predictions. A patient on day 14 with continuous monitoring may have 10,000+ vital sign observations. Running temporal aggregations (min, max, mean, std, trend across 4 time windows for 7+ vital signs = 140+ computations on large arrays) on that volume can push Lambda past 15 minutes. The pre-aggregation pattern solves this: run a scheduled Glue job or SageMaker Processing job that rolls up raw vitals into hourly summaries (mean, min, max per hour), stored in a separate DynamoDB table or S3 prefix. The real-time Lambda then reads the pre-aggregated summaries rather than querying the full raw history. This keeps per-prediction Lambda execution under 30 seconds regardless of ICU length of stay. The fallback behavior matters too: if the Lambda does time out, serve the most recent successful prediction (with its timestamp prominently displayed) rather than returning nothing. A 4-hour-old prediction is better than no prediction, as long as the clinician knows it's stale.
...
**Clinical workflow integration and alert fatigue.** A number on a dashboard is not a clinical intervention. Production requires integration into the EHR workflow: when and how does the prediction surface? Who gets notified? What's the expected clinical action? If the system fires high-risk alerts too frequently (because the threshold is too low, or because the ICU population is inherently high-acuity), clinicians will ignore it. Threshold selection, alert routing, and suppression logic require clinical governance, not just engineering.

**Graceful degradation and stale prediction handling.** When the scoring pipeline fails (HealthLake unavailable, Lambda timeout, SageMaker endpoint cold start), what does the clinical dashboard show? Displaying a stale prediction without a prominent freshness indicator is dangerous: clinicians may make decisions based on a 12-hour-old score that no longer reflects reality. Build a "prediction freshness" indicator (green/yellow/red), define the staleness thresholds with clinical stakeholders, and implement fallback behavior (display last successful prediction with clear age labeling, alert the operations team).

**Calibration drift detection and recalibration cadence.** The local calibration function is trained on recent outcomes, but patient populations change (seasonal respiratory illness surges, new referring hospitals, changes in admission criteria). If calibration drifts and the model says "40% mortality" but the actual rate for patients at that score is 25%, clinicians lose trust. Monitor calibration weekly using a sliding window of recent outcomes. Trigger recalibration when the Hosmer-Lemeshow statistic or expected calibration error exceeds your threshold.

**Model versioning and rollback.** When a retrained model performs worse than its predecessor (detected via shadow scoring or A/B testing), you need to roll back immediately. SageMaker Model Registry supports model versioning, but you need the operational runbook: who approves a rollback, how fast can it happen, and how do you communicate to clinicians that the model version changed?
