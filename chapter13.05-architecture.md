...
**Amazon Neptune for the pathway graph store.** Neptune is AWS's graph database. It speaks two query languages: property graph (Gremlin or openCypher) and RDF (SPARQL). For clinical pathways, property graph wins easily: nodes have typed properties, edges have conditions, and traversal queries read like you'd describe the pathway out loud. Neptune handles graph storage, indexing, and query execution. It's HIPAA eligible, supports encryption at rest and in transit, and scales read replicas for query-heavy workloads. One thing to know: Neptune connections from Lambda add 200-500ms on cold start (WebSocket setup for Gremlin, or HTTP connection for openCypher). For the Traversal Engine Lambda that powers real-time clinical decision support (CDS), configure Provisioned Concurrency of at least 2-5 instances to keep connections warm. Alternatively, use Neptune's openCypher HTTP endpoint for simpler traversal queries to avoid WebSocket overhead entirely.
...
| **DynamoDB Item-Level Access (HIPAA Minimum Necessary)** | The `patient-pathway-state` table contains PHI-adjacent data (patient IDs, pathway positions, timestamps). For HIPAA Minimum Necessary compliance, enforce item-level access control so that Lambda functions processing one department's events cannot read another department's patient records. Use IAM policy conditions with `dynamodb:LeadingKeys` to restrict each department's State Updater Lambda to only the patient IDs it services. Example: add a department prefix to patient IDs (e.g., `CARDIOLOGY-PAT-12345`) and scope IAM with `"dynamodb:LeadingKeys": ["CARDIOLOGY-*"]`. Alternatively, if patient IDs cannot be prefixed, implement application-layer enforcement: the State Updater Lambda validates that the incoming clinical event's department tag matches the patient's assigned department (looked up from a department-mapping table) before reading or writing the pathway state. Choose one approach and document it in your deployment runbook. The IAM leading-key approach is stronger (enforced at the API layer, not bypassable by code bugs) but requires a predictable partition key structure. |
...
This architecture covers the core graph traversal and state management loop. But several production concerns remain unaddressed:

**Health Level Seven (HL7) and Fast Healthcare Interoperability Resources (FHIR) integration testing.** The EventBridge integration assumes clean, well-structured clinical events. Real EHR feeds like HL7v2 admission, discharge, and transfer (ADT) messages and FHIR Subscription notifications arrive with missing fields, duplicate deliveries, out-of-order timestamps, and encoding quirks. You need an integration test suite that replays captured HL7v2 message streams against the State Updater Lambda and validates that pathway state transitions match expected behavior. Budget 3-4 weeks just for edge cases in message parsing.

**Pathway authoring UI.** This recipe assumes pathway definitions arrive as structured JSON. In practice, clinical committees author pathways in narrative documents, Visio diagrams, or proprietary pathway editors. You need a tool that lets clinicians (not engineers) build and validate pathway graphs visually, with guardrails that enforce structural rules (exactly one start node, decision points need multiple branches). Without this, every pathway update requires an engineer to hand-translate a PDF into graph JSON.

**Clinical committee governance workflow.** Pathways change. Evidence evolves, formularies shift, and outcomes data reveals better approaches. Production needs a governance workflow: propose a pathway change, route it through clinical committee review, stage it for testing, validate it against synthetic patients, and then promote it to production, all while patients already on the old version continue uninterrupted. Version management is the easy part; organizational buy-in for the governance process is the hard part.

**Multi-pathway conflict detection.** A patient on three simultaneous pathways can receive contradictory recommendations (one says fluid bolus, another says fluid restriction). This architecture treats each pathway independently. Production needs a conflict detection layer that checks new recommendations against all active pathways before surfacing them to clinicians.

**Graceful degradation under EHR outage.** If the FHIR endpoint goes down, condition evaluation fails. The system should degrade to showing pathway position without real-time condition status, rather than returning errors or stale recommendations without indicating staleness.
