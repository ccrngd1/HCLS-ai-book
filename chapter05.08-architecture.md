...
**Idempotency and retry semantics.** The pipeline must handle duplicate-event delivery, partner-side retries, and Glue job re-runs without producing duplicate encoded records, duplicate match results, or scrambled audit logs. Per-stage idempotency keys: standardize-and-prepare Lambda `(cycle_id, source_record_id)`; bulk-encode Glue `(cycle_id, source_record_id, parameterization_version)`; exchange-encoded-records Lambda `(cycle_id, encoded_record_id, transport_type)`; match-encoded-records Glue or Nitro Enclave `(cycle_id, encoded_record_a_id, encoded_record_b_id)`; disclose-and-route Lambda `(cycle_id, disclosure_event_id, target_consumer)`; invalidate-on-event Lambda `(invalidation_event_source, invalidation_event_id)`; salt-rotation-coordinator Step Functions `(rotation_id, participant_id)`. Configure a dead-letter queue (DLQ) per stage with CloudWatch alarms on DLQ depth (alarm threshold: > 0 records or > 15 minutes for stuck workflows); Step Functions Catch states route terminal failures to the DLQ so stuck workflows are visible. Same chapter pattern as 5.3, 5.4, 5.5, 5.6, 5.7.
...
- [`aws-samples/aws-glue-samples`](https://github.com/aws-samples/aws-glue-samples): Glue extract, transform, and load (ETL) patterns applicable to the bulk-encoding and the matcher-execution jobs
