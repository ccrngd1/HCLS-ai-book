...
**SIG parsing coverage.** The 35 abbreviations in the walkthrough are the common ones. Real-world prescription labels will contain combinations, custom formats, and pharmacy-specific conventions you haven't seen. Build logging around unrecognized tokens in the directions field: capture them, review them regularly, and expand the codebook. This is an ongoing maintenance task, not a one-time setup.

**RxNorm concept selection.** DetectEntitiesV2 returns multiple RxNorm candidate concepts, ranked by confidence. The walkthrough returns all concepts above the threshold. In practice, you often want the highest-confidence single concept for downstream use. Decide whether you want the most specific concept (the one that matches strength and dose form precisely) or the ingredient-level concept (which generalizes across package types). That choice depends on your use case: formulary matching needs the specific concept; interaction checking works at the ingredient level.
