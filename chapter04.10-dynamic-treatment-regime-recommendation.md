...
<!-- TODO (TechWriter): specify the consent-data-flow pattern: explicit consent capture, consent versioning, consent-revocation handling, audit-trail of consent state at the time of regime training and recommendation generation. Mirror the language from 4.5 through 4.9. -->
...
<!-- TODO (TechWriter): replace the string-concatenation recommendation_id, decision_point_id, trajectory_id with opaque, non-reversible identifiers (UUID or HMAC-SHA256 over the composite with a per-environment secret). Trajectory IDs that encode patient identifiers or decision sequences are PHI leakage in URLs, logs, and event payloads. Mirror the language flagged in 4.4 through 4.9. -->
