...
**IAM least-privilege.** Each pipeline phase should use a dedicated IAM role scoped to the specific resource ARNs that phase needs. The Glue feature assembly role gets `s3:GetObject` on source buckets and `s3:PutObject` on the feature store prefix only. The SageMaker role gets `s3:GetObject` on features and `s3:PutObject` on score history only. The Lambda detection role gets `dynamodb:PutItem` on the risk state table and `events:PutEvents` on the specific event bus ARN. Never use a single role with broad permissions across the entire pipeline. Not `s3:*`. Not `AdministratorAccess`. Scoping roles per phase limits blast radius if any component is compromised.
