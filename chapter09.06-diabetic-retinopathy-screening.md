...
**Integration:** Results flow back to the ordering electronic health record (EHR), the patient, and (if referral is triggered) the ophthalmology scheduling system. The complete audit trail (image, model version, prediction, confidence, clinical decision) must be retained for regulatory compliance and quality assurance.
