...
| R-11 | Internal draft markers are live on published pages | Significant | OPEN | Found while assessing R-10, and the executive summary was not the only case. Six published prose pages still carry `TODO`. Two leak the authoring process to readers: `// TODO (TechWriter): Expert review S1 (HIGH)` in `chapter04.10-architecture` and `A7 (MEDIUM)` in `chapter10.09-architecture`. Two are notes to self: verify a MIMIC-IV link (`chapter07.08-architecture`) and add a citation (`chapter15.01-architecture`). One is reader-visible inside a cross-reference in a **main recipe**: `**Recipe 14.3 (TODO: Lab Workflow Optimization):**` in `chapter09.08`. One is arguably legitimate code (`chapter13.06-architecture`). Separately there are 29 prose `TODO`s across the python companions, which matter less since those pages are banner-marked illustrative and out of nav. **Print is unaffected**: its 15 TODOs are all `<!-- PRINT-TODO -->` HTML comments the pipeline generates, and the PDF text layer contains zero. Note `%%` in `chapter07.02-architecture` is Mermaid comment syntax, not a placeholder. |
