...
<!-- DIGEST:BEGIN (generated by tracker_digest.py; do not edit by hand) -->
...
| V-2.8 | Significant | OPEN | Ch12: "discharge order written" is near-label leakage; add point-in-time wa... | Model soundness |
| V-3.8 | Significant | OPEN | Tagged-PDF quality low; no figure/table/TOC tagging | Production and accessibility |
| V-4.4 | Significant | OPEN | Ch11 Honest Take references five bots absent from this volume; other dangli... | Chapter 11 patient safety and stan |
| V-6.2 | Significant | OPEN | No coherent evaluation story (LLM-as-judge reliability, golden datasets, re... | Missing technical layer |
| V-6.3 | Significant | OPEN | No governance artifacts (CHAI model cards, AI use inventories) | Missing technical layer |
| V-6.4 | Significant | OPEN | No LLM security (prompt injection, jailbreaks, PHI in prompts/logs, OWASP L... | Missing technical layer |
| V-6.5 | Significant | OPEN | CDS Hooks missing from the body (Appendix B only) | Missing technical layer |
| V-7.4 | Significant | OPEN | No model change-control or rollback (version pinning in the audit record, c... | Enterprise readiness |
| V-7.5 | Significant | OPEN | No security incident-response or breach-notification runbook for serving ch... | Enterprise readiness |
| V-7.6 | Significant | OPEN | Automation bias unmonitored (no reviewer edit-rate tracking, no seeded errors) | Enterprise readiness |
| V-8.1 | Significant | OPEN | Fairness as a summed penalty can concentrate bad shifts on a few nurses; us... | Chapter 14 optimization |
| V-8.2 | Significant | OPEN | No infeasibility handling despite a scenario that makes it likely; add IIS... | Chapter 14 optimization |
| A-2 | Minor | DEFERRED | About the Author orphan: decide at physical proof | Author-owned work |
| R-9 | Minor | OPEN | `categories/` is tracked in git but is not published and is superseded | Radwin: accessibility, register, s |
| V-1.7h | Minor | OPEN | Ch12: LOS math — bed count is algebraically irrelevant; 8% implies LOS 3.75d | Factual and coding errors |
| V-11.3 | Minor | OPEN | Body chapter titles use recipe names, Appendix A uses capability areas | Positioning, front/back matter, st |
| V-11.4 | Minor | OPEN | Define the header fields in "How to Use This Book" | Positioning, front/back matter, st |
...
| V-10.1 | No About the Author | Critical | DONE | `9bf83650` added the page after the copyright page, first person to match the body voice; `5f228547` moved the prose into `print/frontmatter/about-the-author.md` so it is editable without touching build.py. Renders on page 3. Row was left OPEN after the work shipped, and the digest audit caught it. |
