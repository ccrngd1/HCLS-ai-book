...
**Error handling and dead-letter queues.** If trajectory computation fails for one patient, it must not affect other patients. Production Lambda functions wrap each patient's processing in try/except blocks, send failures to a DLQ for retry, and emit CloudWatch metrics on error rates. A spike in processing errors triggers an operational alarm (distinct from clinical alerts).
