...
1. **API Gateway to recommender Lambda (synchronous).** This is a synchronous request-response path; an SQS dead-letter queue (DLQ) on the Lambda function captures asynchronous invocation failures but doesn't help when API Gateway is the caller. The practical approach: pair structured JSON logging (recommendation_id, patient_id hash, failure reason) with a CloudWatch 5xx alarm on the API Gateway stage, and document a replay-from-logs runbook for incident investigation. Accept the synchronous-API tradeoff (the patient gets an error page, not a silent drop) and focus monitoring effort on the alarm.
