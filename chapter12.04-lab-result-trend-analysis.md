...
**Clinical Consumers.** Surfaced trends flow to consumers: the clinician's inbox, a population-health dashboard, a care coordinator's worklist, or a clinical decision support (CDS) Hooks endpoint that fires during chart open. Each surfaced trend includes the magnitude, the duration, the recent values, the patient's baseline, the lab's reference range, and a plain-language explanation. Note that the surfaced-trend payload is protected health information (PHI) in the strongest sense (patient identifier plus dated lab values plus clinical interpretation); every downstream consumer inherits the PHI posture and must be a HIPAA-eligible service or a business associate agreement (BAA)-covered first-party application.
