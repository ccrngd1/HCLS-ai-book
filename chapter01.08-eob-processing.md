...
Worth saying explicitly, because it affects almost every technical decision. An EOB is a financial record. The fields it contains are claim identifiers, service dates, Current Procedural Terminology (CPT) codes used as billing references, and dollar amounts. When you see "99213" on an EOB, it means the provider billed for this procedure code and here is what was paid. You don't need to understand what 99213 means clinically. You need to extract the code as a string and associate it with the dollar amounts on the same line.
