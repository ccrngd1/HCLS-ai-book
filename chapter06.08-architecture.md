...
**Clinical workflow integration.** A subtype assignment sitting in DynamoDB is useless unless it surfaces where clinicians make decisions. That means integration with the electronic health record (EHR), surfacing as a patient flag, a BPA, or a dashboard element, which requires Health Level Seven (HL7) Fast Healthcare Interoperability Resources (FHIR) interfaces, EHR vendor approval processes, and clinical workflow design to ensure the subtype information appears at the right moment without causing alert fatigue.
