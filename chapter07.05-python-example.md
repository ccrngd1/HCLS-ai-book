...
**Error handling and retries.** Every external call (HealthLake queries, SageMaker endpoint invocation, DynamoDB writes, SNS publishes) needs try/except blocks with exponential backoff. The adaptive retry config helps, but you also need circuit breakers for when a downstream service is fully down. A failed score should never silently disappear; it should land in an SQS dead letter queue for retry. Set a CloudWatch alarm on DLQ depth > 0, run a daily retry Lambda to reprocess failures, and escalate to manual review for any patient unscored after 24 hours.

**Feature assembly from real data sources.** This example generates synthetic features. In production, you'd query HealthLake for the current encounter (diagnoses, procedures, medications, labs) and pull pre-computed historical features from a DynamoDB feature store (keyed by patient_id, populated nightly by Glue). This hybrid approach keeps scoring latency under 500ms even for patients with complex multi-year histories. For patients not yet in the feature store (new to your system), fall back to real-time HealthLake queries for historical data.
