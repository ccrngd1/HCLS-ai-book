...
This is alert fatigue, and it's one of the most dangerous problems in clinical informatics. Peer-reviewed studies over the past two decades, confirmed by recent systematic reviews, consistently report that clinicians override 90 to 96 percent of drug interaction alerts. Not because the alerts are wrong, but because the signal-to-noise ratio is terrible. When everything is flagged, nothing is flagged. The alert that actually matters (warfarin + amiodarone requires a dose reduction) gets the same visual weight as the alert that doesn't (lisinopril has a theoretical interaction that's never been clinically demonstrated at normal doses).

The root cause is that most drug interaction databases are flat lookup tables. Drug A interacts with Drug B: yes or no, with a severity level slapped on. They don't encode the *mechanism* of the interaction, the *clinical context* that makes it relevant, the *patient factors* that modulate risk, or the *evidence quality* behind the claim. A knowledge graph changes this fundamentally. Instead of "these two drugs interact," you can represent "fluconazole inhibits CYP2C9, the liver enzyme that clears warfarin, so adding it to a stable warfarin regimen raises warfarin's effective concentration and pushes the INR up, and that matters more when the patient's hepatic function is already reduced." An interaction of this kind needs one inhibitor and one substrate. Two inhibitors of the same enzyme do not interact with each other by this mechanism at all, which is exactly the sort of distinction a flat lookup table cannot express.
...
[Increased Bleeding Risk] --EVIDENCE--> [RCT: PMID <study id>, level 1]
[Increased Bleeding Risk] --EVIDENCE--> [Product label: <drug>, Drug Interactions]
```
...
**Evidence transparency.** Each interaction path can carry evidence nodes recording what supports it, how strong that support is, and where it came from: a product label's drug-interaction section, a pharmacokinetic study, an observational cohort, or a single case report. The angle brackets in the sketch above are placeholders, not references: what a real evidence node carries is an identifier into whatever source you license or curate, such as a PMID, a DOI, or a section of a product label, together with the evidence level and the date. Storing the identifier rather than a prose citation is what lets you re-check an interaction when the underlying evidence is superseded. A clinician can see *why* the system is alerting and make an informed decision about whether to override.
...
**Buy versus build: license the interaction content, and do not attempt to build it.** This is the clearest buy in the book. Drug interaction knowledge is a licensed commercial product, maintained by dedicated clinical-pharmacology teams who curate it continuously against new literature, new approvals, and new label changes. Recreating that is not an engineering project, it is a permanent clinical-content operation with direct patient-safety liability attached, and no health system should take it on. The free public vocabularies will give you normalized drug identifiers, which is necessary and nowhere near sufficient; they will not give you curated pairwise interactions with severity and evidence grading. What you build, and what this recipe is actually about, is the layer above the licensed content: the patient-specific significance scoring, the dose and route context, the alert suppression, and the override analytics. Your vendor sells you interactions. Nobody sells you a system that alerts only when the interaction matters for this patient at this dose. That part is yours.

**Alert fatigue is the real enemy, not missing interactions.** Any alerting platform I've seen starts with the goal of "catch everything" and ends up being ignored because it catches too much, a prescription warning system is no different. The hard engineering problem isn't finding interactions. It's *not* alerting on the ones that don't matter for this specific patient at this specific dose. Your significance scoring algorithm will get more engineering attention than your graph traversal logic.
...
**Mechanism-based inference is powerful but imperfect.** Inferring that "Drug X inhibits CYP3A4, Drug Y is a CYP3A4 substrate, therefore they interact" is pharmacologically sound but clinically oversimplified. The clinical significance depends on how much of Drug Y's metabolism goes through CYP3A4 (if it's only 10%, the interaction is negligible), the therapeutic index of Drug Y, and the strength of inhibition. Your inference engine needs these nuances or it will generate too many false positives.
