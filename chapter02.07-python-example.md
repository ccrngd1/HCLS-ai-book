...
**DynamoDB Decimal gotcha.** DynamoDB does not accept Python floats. The example wraps numeric fields through `str` and then `Decimal` where needed; the common trap on first deployments is passing a float directly and getting `TypeError: Float types are not supported`. Always route floats through `Decimal(str(value))`; going through `str` avoids the binary-precision issues that `Decimal(float_value)` introduces.
