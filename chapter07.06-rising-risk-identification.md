...
**Claims lag and the incomplete recent window.** If any of your trajectory features come from claims, the most recent window is a trap. A claim can take weeks or months to be submitted and adjudicated, so at scoring time the last month or two of claims-derived utilization is systematically undercounted, while the historical data you trained on had long since matured and looked complete. A rising-risk detector is the worst place for this: the recent window is exactly what signals the rise, and understating it makes a deteriorating patient look flat or even improving, so the model misses the patients it exists to catch. Build the trajectory features point-in-time correct, computing each training example as of its scoring date using only the claims that would have arrived by then, or draw the recent window from data without the lag, the electronic health record (EHR) and admission, discharge, and transfer (ADT) feed, and reserve claims for the mature history.
