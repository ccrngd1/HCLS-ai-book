...
The AWS implementation borrows heavily from Recipes 12.1 and 12.2; the platform pieces (managed training, batch inference, scheduled orchestration, low-latency serving) are the same. What changes is the cadence (hourly retraining and inference, not nightly), the streaming ingest path for admission, discharge, and transfer (ADT) messages, and the integration with operational ED dashboards.
...
**Catch-and-route to SQS dead-letter queue (DLQ).** On persistent failure at any stage, the Catch block routes the failed input (stage name, input payload, error type) to an SQS dead-letter queue (`ed-forecast-inference-dlq`). A CloudWatch metric (`inference_stage_failed`) increments with dimensions `(ed_id, stage_name)`.
