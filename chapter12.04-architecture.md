# Recipe 12.4 Architecture and Implementation: Lab Result Trend Analysis

*Companion to [Recipe 12.4: Lab Result Trend Analysis](chapter12.04-lab-result-trend-analysis). This page covers the AWS architecture, services, prerequisites, and pseudocode. For the problem framing and the conceptual approach, start with the main recipe.*
...
<!-- TODO (TechWriter): Expert review A8 / M7 (MEDIUM). Add a CDS Hooks responder to the architecture (API Gateway + Lambda, both VPC-deployed and behind WAF) wired to the DynamoDB serving table. The Lambda implements the CDS Hooks specification, queries DynamoDB by patient_id on the patient-view hook, formats the surfaced trends as CDS Hooks cards, and returns them with appropriate severity-band styling. Show this path in the architecture diagram; the EHR's CDS Hooks client connects to the API Gateway endpoint over a HIPAA-eligible mutual-TLS or signed-JWT channel. -->
...
<!-- TODO (TechWriter): Expert review A2 / H3 (HIGH). See also the Step 2 marker. This bullet identifies the failure mode; the architecture in the AWS Implementation section should be updated with the explicit baseline-reset event feed, the post-reset stabilization window, and the change-point backstop. -->
