...
**Risk Stratification.** Convert the raw probability into an actionable tier (high, medium, low) using the calibrated probability itself, not a percentile rank. This is where the calibration argument earns its keep: define the tiers by probability (for example, above 30% is high risk and gets intensive intervention, 15 to 30% is medium, below 15% is routine), so a tier boundary is a real risk level a care team can reason about. The pull toward rank is strong, because a team with room for 50 patients a week is tempted to take the top 50 by score, but that discards the calibrated numbers the section above argued for and re-tiers every patient each week on relative position. Set the probability thresholds to produce a volume your team can handle and adjust them as capacity changes, rather than letting rank define risk.
