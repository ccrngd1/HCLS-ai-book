...
For each forecast hour and each candidate admit unit, the pipeline produces an expected admission count plus a sample distribution suitable for Monte Carlo composition. Different sources use different methods: ED-driven admissions run through a Poisson model, direct admits and transfers run through a count model (the demo uses a Poisson sampler for readability; production uses negative binomial for these burstier, over-dispersed streams), the OR schedule is read deterministically and multiplied by show-rate, and the transfer queue contributes a near-term known quantity plus a modeled tail. Unit assignment runs through a multinomial classifier given the admission's source.
...
*← [Recipe 12.5: Hospital Census Forecasting](chapter12.05-hospital-census-forecasting) · [Architecture and Implementation companion](chapter12.05-architecture) · [Chapter 12 Index](chapter12-preface) · [Next: Recipe 12.6 - Revenue Cycle Cash Flow Forecasting →](chapter12.06-revenue-cycle-cash-flow-forecasting)*
