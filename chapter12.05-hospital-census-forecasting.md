...
*ED admissions* are the largest single source for most hospitals (roughly half of all inpatient admissions nationally, per RAND and HCUP data, and higher in community hospitals than in academic centers, which carry more scheduled and transfer volume). They follow patterns similar to ED arrivals, with the additional layer of admit-rate variability. About 12 to 14% of ED visits become admissions nationally (per CDC's National Hospital Ambulatory Medical Care Survey), though the rate fluctuates with case-mix, season, and ED capacity itself. Forecast volume by ED arrival rate times admit rate, both modeled with the same calendar and weather features used for ED-arrival forecasting.

*Scheduled surgical admissions* are the most predictable inflow because they're literally on a calendar. Tomorrow's OR schedule produces a deterministic count of inpatient post-op admits for the next 24 to 48 hours, with high reliability. The only uncertainty is no-show rate (typically 2 to 5%) and case-cancellation rate (5 to 12% on the day-of, depending on hospital), both consistent with published operating-room no-show and day-of-surgery cancellation studies. Forecast: pull the OR schedule, multiply by the historical show-rate, attribute to the appropriate post-op unit.

*Direct admissions* (admitted from a physician office or clinic without going through the ED) are smaller volume but harder to forecast because the lead time is short (often hours, not days) and the demand is bursty. A fall-day spike in direct admits to ortho is hard to anticipate from yesterday's pattern. Because the demand is bursty, most hospitals model these with negative binomial regression (a better fit for overdispersed, bursty counts than Poisson, because it lets the variance exceed the mean and so produces more honest prediction intervals) using day-of-week and seasonality features.
...
**Length of stay is the most operationally actionable input.** A 0.3-day reduction in average length of stay (LOS) frees up roughly 8% of bed-hours in a 412-bed hospital. The forecast has to be sensitive enough to detect when LOS is shifting and surface that to the leadership who can act on it. A pipeline that forecasts census without ever exposing the LOS assumption is missing the operational lever the bed huddle most wants to pull.

**The data is mostly timestamped, but timestamps are not always trustworthy.** Admission, discharge, and transfer (ADT) messages timestamp every state change in the patient's hospital journey. In theory, you can reconstruct the entire hospital flow from ADT history. In practice, ADT timestamps are sometimes wrong (the discharge timestamp captures when the discharge order was entered, not when the patient left the bed; the transfer timestamp captures when the patient was assigned to the new unit, not when they actually moved). Production pipelines clean and reconcile timestamps against secondary signals (bed-cleaning logs, electronic health record (EHR) location updates, telemetry leads detached) and treat the cleaned version as authoritative. Skipping this work produces a forecast trained on noisy timestamps that is calibrated to the noise rather than to the underlying flow.

### Methods That Earn Their Keep

For each of the three layers, a small number of methods consistently work in production.

**For inflow forecasting (Layer 1):** The same toolbox used for ED-arrival forecasting, plus deterministic schedule integration for surgical admits. Poisson regression with calendar and weather features handles ED-driven admissions, but test that assumption rather than inheriting it: once you condition on calendar features, ED admit counts are often overdispersed too, and if the residual variance exceeds the mean you want negative binomial here for the same reason you want it for the burstier streams below. Poisson is the starting point, not the answer. Multinomial classifiers handle service-line and unit assignment. The OR schedule is read directly. Direct admits and transfer requests are burstier, so they use negative binomial models with day-of-week features.
...
**For census composition (Layer 3):** Bookkeeping plus Monte Carlo. Sample N times from each component distribution (admissions, discharges, transfers), compose the census trajectory for each sample, take the mean and percentiles across samples. This naturally produces calibrated prediction intervals. Monte Carlo (and its close cousin, discrete-event simulation) is a long-established standard for hospital bed-occupancy and patient-flow modeling, so the composition step here is well-trodden ground rather than a novel technique. A hospital with thousands of patient-state combinations runs this comfortably with N=500 to 1000 samples; per-unit calibration improves with more samples but with diminishing returns past 1000.
...
**Buy versus build: build the forecast, and check whether you have already bought one.** Census forecasting from your own ADT feed is genuinely tractable with open-source tooling, and the model is, as noted below, the smallest line item in the project. But capacity-management platforms are widely deployed and most of them include forecasting, so the first question is whether your institution already owns a forecast that nobody has turned on, validated, or trusted. If so, evaluate it against a naive baseline before building a competitor, and if it is adequate, put your effort into the operational integration instead. Where you do build, you are building against your own ADT quirks, your own unit definitions, and your own discharge patterns, none of which a vendor can model better than you can. The thing worth buying, if anything, is the nurse-facing workflow surface. The thing worth building is the forecast and the narrative layer that makes it actionable.

The forecasting math is the easy part of this. The hard parts are upstream (ADT cleanup, length-of-stay feature engineering, unit-assignment ground truth) and downstream (operational integration, feedback capture, narrative explanation). Teams routinely walk in expecting the modeling to be the bulk of the work; in practice it is closer to the smallest line item, and the budget goes mostly to the data plumbing upstream and the operational integration downstream.
