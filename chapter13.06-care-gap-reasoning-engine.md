...
**Procedure code mapping.** Completed services show up as Current Procedural Terminology (CPT) codes, and the same problem repeats on the procedure side: a guideline that says "screening colonoscopy every 10 years" has to match against a specific procedure code range, and the reasoner needs to know which codes count as a screening colonoscopy versus a diagnostic one.
