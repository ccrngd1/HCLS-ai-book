...
Even within a single document type — say, insurance cards — every payer has their own design. Blue Cross Blue Shield layouts look nothing like Aetna layouts, which look nothing like a regional Medicaid managed care plan's card. The fields are in different positions, use different fonts, have different visual relationships to each other. Some put the member ID prominently at the top. Some bury it in the bottom left. Some use horizontal layouts, some vertical. Some cards are plastic and look pristine; some are paper-laminated and have been sitting in a wallet for three years.
