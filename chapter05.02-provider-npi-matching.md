...
The assignment record should carry the matched Type 1 NPI as the primary anchor plus a list of current Type 2 affiliations. Each affiliation entry carries the affiliated organization name, the Type 2 NPI, an effective date range, and a primary-billing flag indicating whether the provider currently bills primarily through that entity. Drift detection should surface `type2_affiliation_added` and `type2_affiliation_removed` as separate event types from Type 1 drift, because different downstream consumers care about different changes: claims-validation subscribes to Type 2 affiliation events (because billing NPIs are Type 2), network-adequacy reporting subscribes to both (because affiliation determines which organizations the provider practices under), and the credentialing system subscribes primarily to Type 1 events (because the individual identity is what gets credentialed).
...
The operational thresholds, inheriting the rigor from 5.1's cohort-monitoring framework:

- **Match-rate disparity threshold: 0.10.** If any cohort's auto-attach rate differs from the population mean by more than 10 percentage points, an alert fires.
- **Auto-attach precision disparity threshold: 0.05.** If any cohort's auto-attach precision (measured against the gold set) differs from the population mean by more than 5 percentage points, the cohort's thresholds need recalibration.
- **Review-queue depth-per-FTE disparity: 0.20.** If any cohort contributes more than 20% above its expected share of the review queue (normalized by provider volume), investigate whether the blocking passes or comparators are systematically under-performing for that cohort.
- **Post-match drift-rate disparity: 0.05.** If any cohort's drift-detection rate differs from the population mean by more than 5 percentage points, investigate whether the underlying data sources update at different rates for that cohort.
- **MIN_COHORT_SAMPLE_SIZE: 100 per measurement window.** Provider volumes are smaller than patient volumes, so the sample floor is lower than in recipe 5.1 (which uses 200). The rationale: a cohort with fewer than 100 providers in a measurement window produces confidence intervals too wide to detect meaningful disparities; suppress alerts for under-sized cohorts and flag them for manual review at the quarterly gold-set refresh instead.

Each metric supports per-axis-per-metric override (e.g., the rural-geography axis might tolerate a 0.12 match-rate disparity because address standardization is inherently harder in rural areas, while the naming-convention axis holds at 0.10). Cohort-stratified gold sets should be constructed with deliberate representation: sample providers from each monitored cohort in proportion to their network share, with a floor of MIN_COHORT_SAMPLE_SIZE per cohort. The diagnose-and-address workflow that fires on threshold crossings: (1) identify the affected cohort and metric, (2) pull the cohort's recent match records and review decisions, (3) run the per-field comparator analysis to identify which field or blocking pass is under-performing, (4) propose a targeted fix (supplementary blocking pass, per-cohort m/u adjustment, comparator tuning), (5) validate the fix against the cohort's gold-set partition, (6) deploy with a canary window before promoting to full traffic.
